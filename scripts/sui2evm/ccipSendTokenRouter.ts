...
import { buildTokenTransferPTB, type TokenTransferArgs } from './core/buildCcipSendPTB';
import {
  getSuiClient,
  getSuiConfig,
...
async function sendTokenFromSuiToEvm(tokenAmount: number) {
  try {
    ...
    // Prepare message data(empty for token-only transfer)
    const data = prepareMessageData("");

    // Prepare extra args (Zero gas limit for token-only transfer)
    ...
    buildTokenTransferPTB(tx, buildArgs);
...
sendTokenFromSuiToEvm(argv.amount);