import { ethers, hexlify, Interface, toUtf8Bytes, zeroPadValue } from "ethers";
...
import { getEvmChainConfig, handleError, isValidSuiAddress, encodeSuiExtraArgsV1, suiObjectIdToBytes32, sendCcipWithGasFallback, applyGasEstimateFallback } from "../utils/utils";
...
const argv = yargs(hideBin(process.argv))
    ...
    .option("suiReceiver", {
        type: "string",
        description: "Specify the Sui Receiver address (Package ID of the Receiver intended to receive message)",
        demandOption: true,
    })
    .parseSync();
...
function buildCCIPMessage(
    ...
    return [recipient, data, [], feeToken, extraArgs];
}
...
async function sendMessagePayLink(
    ...
    try {
        ...
        const ccipMessage = buildCCIPMessage(
            recipient, // Sui CCIP receiver package address (i.e., ccip_message_receiver package in our case)
            hexlify(toUtf8Bytes(messageString)), // test data, can be any valid hex string
            feeTokenAddress,
            encodeSuiExtraArgsV1({
                gasLimit: 100_000n,
                allowOutOfOrderExecution: true,
                tokenReceiver: ethers.ZeroHash, // No token transfer, so tokenReceiver should be zero bytes32
                receiverObjectIds: [
                    suiObjectIdToBytes32(suiConfig.clockObjectId), // Clock object ID (0x6 padded to 32 bytes)
                    await getObjectFromPackage(recipient, 'CCIPReceiverState', suiNetworkName)
                ]
            })
        )
...
async function sendMessageFromEvmToSui(messageString: string) {
...
sendMessageFromEvmToSui(argv.msgString);